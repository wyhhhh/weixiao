                    ...
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal m-t" id="mainForm" data-NoClientValidate="1">
                            <div class="form-group">
                                <label class="col-sm-3 control-label">楼栋单元楼层数</label>
                                <div class="col-sm-8">
                                    <select style="width: 150px;" class="form-control" name="floorid" id="floorid" required="true">
                                        <option selected="" value="">请选择楼栋单元楼层数</option>
                                        <?php if(is_array($rolelist)): $i = 0; $__LIST__ = $rolelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vol["id"]); ?>" <?php if(($vol["id"]) == $info["floorid"]): ?>selected='selected'<?php endif; ?> ><?php echo ($vol["name"]); ?>-<?php echo ($vol["uname"]); ?>-<?php echo ($vol["fname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input placeholder="请输入楼层" name="name" class="form-control" type="text" required="true" number='true' value="<?php echo ($info["name"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">房屋面积</label>
                                <div class="col-sm-8">
                                    <input placeholder="请输入楼层" name="house_area" class="form-control" type="text" required="true" number='true' value="<?php echo ($info["house_area"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">房号全</label>
                                <div class="col-sm-8">
                                    <input placeholder="请输入房号全(参考楼栋单元楼层数)" name="number" class="form-control" type="text" required="true"  value="<?php echo ($info["number"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">物管缴费截止日期</label>
                                <div class="col-sm-8">
                                    <input placeholder="无人入住不填" name="pay_fees_deadline" class="form-control" type="text" value="<?php echo ($info["pay_fees_deadline"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
