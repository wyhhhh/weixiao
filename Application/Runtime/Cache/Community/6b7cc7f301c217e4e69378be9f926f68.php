                    ...
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal m-t" id="mainForm" data-NoClientValidate="1">
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input placeholder="请输入车位号" name="number" class="form-control" type="text"  value="<?php echo ($info['number']); ?>" required="true" > 
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">车位所有人</label>
                                <div class="col-sm-8">
                                    <select style="width: 150px;" class="form-control" name="userid" id="userid" required="true">
                                        <option selected="" value="">请选择车位所有人</option>
                                        <?php if(is_array($alias)): $i = 0; $__LIST__ = $alias;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$volaalias): $mod = ($i % 2 );++$i;?><option value="<?php echo ($volaalias["id"]); ?>" <?php if(($volaalias["id"]) == $info["userid"]): ?>selected='selected'<?php endif; ?> ><?php echo ($volaalias["real_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input placeholder="请输入车牌" name="license_plate" class="form-control" type="text" required="true" number='true' value="<?php echo ($info["license_plate"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">车位状态</label>
                                <div class="col-sm-8">
                                    <select style="width: 150px;" class="form-control" name="status" id="status" required="true">
                                        <option selected="" value="">请选择车位状态</option>
                                        <?php if(is_array($sta)): $i = 0; $__LIST__ = $sta;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vola): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vola["id"]); ?>" <?php if(($vola["id"]) == $info["status"]): ?>selected='selected'<?php endif; ?> ><?php echo ($vola["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <select style="width: 150px;" class="form-control" name="paystyle" id="paystyle" required="true">
                                        <option selected="" value="">请选择缴费方式</option>
                                        <?php if(is_array($pay)): $i = 0; $__LIST__ = $pay;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vol["id"]); ?>" <?php if(($vol["id"]) == $info["paystyle"]): ?>selected='selected'<?php endif; ?> ><?php echo ($vol["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">该缴金额</label>
                                <div class="col-sm-8">
                                    <input placeholder="请输入该缴金额" name="paymoney" class="form-control" type="text" required="true" number='true' value="<?php echo ($info["paymoney"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-3 control-label">物管费</label>
                                <div class="col-sm-8">
                                    <input placeholder="请输入物管费" name="manage_fees" class="form-control" type="text" required="true" number='true' value="<?php echo ($info["manage_fees"]); ?>">
                                </div>
                            </div>
                            <div class="form-group">
