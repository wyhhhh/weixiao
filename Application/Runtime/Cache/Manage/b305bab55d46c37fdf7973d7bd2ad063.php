...
<html>
	...
	<head>
		...
		<title>主贴管理</title>
		<meta name="keywords" content="主贴管理">
		<meta name="主贴管理">
...
<script src="/wuyeguanli/Public/hplus/js/content.js" charset="UTF-8"></script>
		...
		<script type="text/javascript">
			var url = "<?php echo U('ajax_getList',array('key'=>'1'));?>";
			// 搜索 （根据条件重新加载数据）
				...
				$('#tablelist').bootstrapTable($.extend(true, bootstrap_table.datagridDefaultConfig, {
					...
					},
					{
						...
						formatter: function(value, row, index) {
							...
							var filed_url = "<?php echo U('ajax_field_edit');?>?status="+newstatus+"";
							var unset = "<a data-val='" + newstatus + "' data-id='" + id + "' href='javascript:void(0)' data-uri='" + filed_url + "'   onclick='updateField(this)' data-msg='你确认要修改" + name + "的审核状态吗？' >" + status_text + "</a>";
							strs += " " + unset;
							
							//修改置顶状态
							var oldistop = row.istop; 
							// var newistop = 0;
							// var istop_text = "不置顶";
							if(oldistop == 0)
							{
							...
							var edit_filed_url = "<?php echo U('ajax_field_edit');?>?istop="+newistop+"";
							var edit_filed = "<a data-val='" + newistop + "' data-id='" + id + "' href='javascript:void(0)' data-uri='" + edit_filed_url + "'   onclick='updateField(this)' data-msg='你确认要修改" + name + "的置顶状态吗？' >" + istop_text + "</a>";
							strs += " " + edit_filed;

							var repairUrl = "<?php echo U('Post/index2');?>?id=" + id; 
							var repair = '&nbsp;<a href="javascript:void(0);" onclick="showDialog(this)"  data-uri="' + repairUrl + '" data-title="当前帖子回帖记录" data-width="1200" data-height="800" >查看回帖</a>&nbsp;';
							strs += " " + repair;
							//查看详情
