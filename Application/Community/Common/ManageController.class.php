...
class ManageController extends SystemController
{
	...
	{ 
        ...
        $info = M('ManagesCommunity')->field('m.id,m.status,mr.name as rolename,m.username,mr.permissions')->join(" as m inner join ".$this->_qz."manage_role as mr on mr.id = m.roleid ")->where($map)->find();
        if (empty($info))
        {
            if (IS_AJAX)
            {
